    ...
    for term in query_terms:
        ...
        if term not in seen_terms:
            ...
            # else, load the term from file and add it to all_terms
                ...
                all_terms[term] = {}
                try:
                    with open("IndexPickle/" + term + ".p", "rb") as iFile:
                        term_temp = pickle.load(iFile)
                except FileNotFoundError:
                    continue
                # gather docs in posting list and gather champs
                for id, vals in term_temp.items():
    ...
    for term in seen_terms:
        ...
        for id in champs:
            ...
            if id in term_index[term]:
                ...
                if isTitle:
                    score_dict[id] += tf * idf * w_tq[term][id] * 1.5
                elif isHead:
                    score_dict[id] += tf * idf * w_tq[term][id] * 1.25
                elif isStrong:
                    score_dict[id] += tf * idf * w_tq[term][id] * 1.25
                else:
                    score_dict[id] += tf * idf * w_tq[term][id]
...
#    global all_terms
#    ps = PorterStemmer()
#    with open("long_postings_lists_pickle.p", "rb") as iFile:
...
if __name__ == "__main__":
    ...
    for term in stop_list:
        stop_dict[term] = 0
    doc_id = dict()
    with open("docIDPickle.p", "rb") as iFile:
