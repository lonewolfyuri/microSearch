...
def calculate_idfs(term):
    ...
    try:
        with open("Index/" + term + ".txt", "r") as iFile:
            term_index = eval(iFile.read())
            sz = len(term_index)
            tfidfs = {}
...
#        token_count += 1
    ...
    with open("long_postings_lists.txt", "w") as oFile:
        oFile.write(str(long_postings_lists))
